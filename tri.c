...
void   ReadVertices( void )
{
   ...
   int      vnum = 0;
   int n;
   printf("Welcome to Art Gallery min vertex guards Solver\n");
 ...
 printf("processing\n");
   ...
   nvertices = vnum;
   if (nvertices < 3) 
      printf("Error in No of Vertices: vertices=%d <3\n", nvertices),
      exit(EXIT_FAILURE);
}
/*---------------------------------------------------------------------
...
if (count1<count2)
	{
		...
		{
		large=1;
		printf("min no of guards required :%d" ,count1);
	}else{
		large=3;
		printf("min no of guards required :%d" ,count3);
	}

}else if (count2<count3)
{
	large=2;
	printf("min no of guards required :%d" ,count2);
}
else{
	large=3;
	printf("min no of guards required :%d" ,count3);
...
printf("\n");
   ...
   for (i = 0; i < count; i++)
    {
    if(color[i]==large){
    	printf("Guard should be placed on vertex : %d\n",i );
    }
